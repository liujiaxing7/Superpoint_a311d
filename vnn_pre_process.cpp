...
*/
vsi_status vnn_PreProcessSuperpointV1_ImageProcess
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
    )
{
    ...
    {
        printf("vnn_PreProcessSuperpointV1_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
...
vsi_status vnn_PreProcessSuperpointV1
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
